...
(async () => {
	...
	for (const entryName in entries) {
		...
		if (!entry.isDirectory) {
			if (path.extname(entryName) === '.json' && !entryName.startsWith('media/')) {
				// Root JSON file
	...
	for (const fileName of moduleFiles) {
		...
		if (stat.isFile() && path.extname(filePath) === '.json') {
			...
			const search = (node: any) => {
				if (Array.isArray(node)) {
					...
					for (const key in node) {
						...
						if (typeof node[key] === 'string' && (key === 'src' || key === 'imgSrc')) {
			...
			importJsonLines.push(`const ${moduleName} = require('@/assets/modules/${fileName}');`);
			moduleObjects.push(`{
		id: '${moduleId}',
		title: '${moduleTitle}',
		imgSrc: '${moduleImg}',
